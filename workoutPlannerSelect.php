...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["plan_id"]))){
        $plan_id_err = 'Please enter plan_id.';
    } else{
	...
	$sql = "SELECT * FROM workout_plan where user_id = $user_id AND plan_id=$plan_id";
	$result = $link->query($sql);
	if ($result->num_rows > 0) {
		echo "<table><tr><th>user_id</th><th>workout_id</th><th>workout_date</th><th>amount</th></tr>";
		// output data of each row
		while($row = $result->fetch_assoc()) {
			echo "<tr><td>" . $row["user_id"]. "</td><td>" . $row["workout_id"]. "</td><td>" . $row["plan_id"]. "</td><td>" . $row["plan_amount"]. "</td></tr>";
		}
		echo "</table>";
	} else {
		echo "0 results";
	...
	if(!(empty($user_id_err))){
		...
		header("location: indexLogin.html?u_err=".$plan_id_err); // send error message in the err variable
	}
